...
const EntertainmentCardSlider = () => {
  ...
  const getSlugFromUrl = (url) => {
    // Extracts 'workshop-and-more' from '...w-300/workshop-and-more-web-collection-202211140440.png'
    const parts = url.split("/");
    const filename = parts[parts.length - 1];
    return filename.split("-collection")[0].split("-web")[0];
  };
    ...
    <>
      <Slider {...settings}>
        {EntertainmentImage.map((image, index) => (
          <EntertainmentCard
            src={image}
            to={`/category/${getSlugFromUrl(image)}`}
            key={index}
          />
        ))}
      </Slider>
